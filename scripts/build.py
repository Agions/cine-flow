...
def build_exe(platform="auto"):
    ...
    subprocess.run(cmd)
    print(f"✓ 构建完成: dist/{platform}/NarrateFlow")
...
def create_portable():
    ...
    version = "1.0.1"
    archive_name = f"NarrateFlow-{version}-portable.zip"
    
    with zipfile.ZipFile(archive_name, 'w', zipfile.ZIP_DEFLATED) as zf:
        # 添加可执行文件
        dist_path = Path("dist/windows/NarrateFlow")
        if dist_path.exists():
...
def main():
    ...
    parser = argparse.ArgumentParser(description="NarrateFlow 构建工具")
