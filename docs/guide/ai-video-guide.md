---
title: AI 视频解说工作流
description: 深入了解 NarrateFlow AI 第一人称视频解说全流程的工作原理与使用技巧。
---

# AI 视频解说工作流

本文档详细介绍 NarrateFlow **AI 第一人称视频解说**（MonologueMaker）的工作流程，帮助你理解每个环节的原理，从而更好地掌控输出质量。
...
NarrateFlow 使用 **Qwen2.5-VL（7B）** 逐帧分析视频画面。
...
NarrateFlow 内部构建的 Prompt 结构（用户不可见）：
...
Edge-TTS 提供逐词时间戳（word-level timing），精确到每个字的起止时间，NarrateFlow 将其用于 ASS 字幕生成。
...
NarrateFlow 支持将原片音频保留为背景音（可选），配音音量自动降低原音：
